...
// Parameters:
//   void *parm - A pointer to a struct threadParm, which contains the necessary
//                information for the producer thread, including the file name to read
//                from and the thread number.
//
//-
void * producer(void * parm){
    // Cast the void pointer to a pointer of type struct threadParm
    struct threadParm *prodParm = (struct threadParm *) parm;

    // Define the maximum length of a line that can be read from the file
    const unsigned int linelen= 1024;

    // Buffer to store a line read from the file
    char line[linelen];

    // Variables to keep track of the current line number and the integer value read from the line
    int lineNo = 0;
    int value = 0;

    printf("Enter producer %d\n",prodParm->threadNum); // Print a message indicating that this producer thread has started
    ...
    // write code to implement the producer based on comments
    // Read lines from the file until the end of the file is reached
    while(fgets(line, linelen, inFile)){
        lineNo++; // Increment the line number counter
        value = atoi(line);  // Convert the line read from the file to an integer

        // lock
        pthread_mutex_lock(&mutex); // Convert the line read from the file to an integer
    ...
    printf("Exit producer %d\n",prodParm->threadNum); // Print a message indicating that this producer thread has finished
...
// Parameters:
//   void *parm - A pointer to a struct threadParm, which contains the necessary
//                information for the consumer thread, including the file name to write
//                to and the thread number.
//
//-
void * consumer(void * parm){
    ...
    // Variables to keep track of the current line number and the integer value read from the buffer
    int lineNo = 0;
    int value = 0;

    // Variable to store the location in the buffer from which the value was read
    int location;
    ...
    // write code to implement the consumer based on comments
    while(1){
        lineNo++; // Increment the line number counter
        ...
        value = buffer[tail];
        tail = (tail + 1) % numSlots;// Store the location from which the value was read
        numElements--;
    ...
    printf("Exiting consumer %d\n",consParm->threadNum); // Print a message indicating that this consumer thread has finished
    return NULL;
}
